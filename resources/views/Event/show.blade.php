...
@section('content')
    ...
    <a href="{{route('event.edit',$event)}}" class="btn btn-danger" data-toggle="modal" data-target="#modalDeleteEvent">Supprimer</a>
...
<div class="modal fade" id="modalDeleteEvent" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
    <div class="modal-dialog">
      <div class="modal-content bg-danger">
        ...
        <div class="modal-body text-center">
          <p>Vous êtes sur le point de supprimer cet evenement! <br> Cette action n'est pas reversible!</p>
        </div>
        <div class="modal-footer float-right">
          ...
          <form action="{{route('event.destroy',$event)}}" method="post">
            @csrf
            @method('DELETE')
            <button type="button" class="btn btn-outline-light">Supprimer</button>
        </form>
        </div>
      </div>
      <!-- /.modal-content -->
